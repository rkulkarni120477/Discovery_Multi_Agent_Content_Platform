...
from app.manual_graph_scenario1 import get_compiled_graph
from app.manual_status import ManualRunStatusResponse, build_status
from app.state_scenario1 import PHASES, TOTAL_STEPS

router = APIRouter(prefix="/manual/scenario1", tags=["manual-scenario1"])
...
def create_run(request: RunCreateRequest):
    initial = {"job_id": request.job_id, "status": "running", "documents": {k: getattr(request, k).model_dump() for k in ("existing_product_content", "sc_standards_reference", "sc_performance_targets", "sc_vertical_articulation")}}
...
def approve(run_id: str):
    ...
    if current.status != "paused" or current.interrupt_type != "manual_approval":
        raise HTTPException(status_code=409, detail="manual run is not waiting for step approval")
...
def resume(run_id: str, request: ResumeRequest):
    current = status(run_id)
    if current.status != "paused" or current.interrupt_type == "manual_approval":
        raise HTTPException(status_code=409, detail="manual run is not waiting for a review decision")
